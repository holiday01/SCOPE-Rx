"""
T3e — Oncology / phase / MOA filter (Patch 2).

Reads T3d output (cohort drug_score, per-patient scores) and produces:
  1. An 'oncology_relevance' score per drug based on PRISM metadata:
       +3 if disease.area contains oncology/cancer/tumor/malignancy/leukemia/lymphoma
       +2 if indication mentions cancer/tumor/carcinoma/sarcoma/lymphoma/leukemia
       +2 if any target in drug.target is a known cancer driver
       -3 if moa in non-oncology blacklist (surfactant, antiseptic, antimicrobial, ...)
       -2 if phase == 'Preclinical'
  2. A strict oncology-filtered ranking (keep only oncology_relevance > 0 AND phase ≥ Phase 1).
  3. A soft-combined ranking (original score × sigmoid(oncology_relevance)).

Outputs:
  results/t3e/drug_oncology_metadata.parquet
  results/t3e/drug_score_filtered_strict.parquet
  results/t3e/drug_score_soft_combined.parquet
  results/t3e/comparison_top20.md
"""
...
OUT  = ROOT/'results/t3e'; OUT.mkdir(parents=True, exist_ok=True)
def log(m): print(f'[{time.strftime("%H:%M:%S")}] {m}', flush=True)

# Cancer driver genes from COSMIC Tier-1 + commonly actionable targets
CANCER_DRIVERS = set("""
...
ATR ATM RAD51 RAD52 DNA-PK BRD2 BRD3 BRCA PARP HDAC BET
""".split())

# Blacklist MOAs for non-oncology / non-specific cytotoxic
...
ONC_DISEASE_PATTERNS = r'oncology|cancer|tumor|tumour|malignan|leukemia|leukaemia|lymphoma|carcinoma|sarcoma|myeloma|melanoma|glioma|glioblastoma|neoplasm|metastas'
ONC_RE = re.compile(ONC_DISEASE_PATTERNS, re.IGNORECASE)
...
def onc_relevance(row):
    score = 0; flags = []
    da = str(row.get('disease_area') or '')
    ind = str(row.get('indication') or '')
    moa = str(row.get('moa') or '')
    tgt = str(row.get('target') or '')
    phase = str(row.get('phase') or '')
    if ONC_RE.search(da):
        score += 3; flags.append('disease_area_onc')
    if ONC_RE.search(ind):
        score += 2; flags.append('indication_onc')
    tgt_hits = [t.strip() for t in re.split(r'[,;/\|\s]+', tgt.upper()) if t.strip() and t.strip() in CANCER_DRIVERS]
    if tgt_hits:
        score += 2; flags.append(f'driver_target({",".join(tgt_hits[:3])})')
    if moa and NONSPEC_RE.search(moa):
        score -= 3; flags.append('nonspec_moa')
    if phase == 'Preclinical':
        score -= 2; flags.append('preclin')
    elif phase == 'Launched':
        score += 1; flags.append('launched')
...
log(f'oncology_relevance distribution: {raw.onc_relevance.describe().round(2).to_dict()}')
log(f'  score ≥ 3: {(raw.onc_relevance>=3).sum()}  (strict oncology-tagged)')
log(f'  score > 0: {(raw.onc_relevance>0).sum()}  (permissive)')
log(f'  score ≤ -3: {(raw.onc_relevance<=-3).sum()}  (non-onc cytotoxic)')

# ---------- 3. Merge with T3d scores ----------
...
merged = cohort.merge(raw[['drug_lc','moa','target','phase','indication','disease_area','onc_relevance','onc_flags']],
                      on='drug_lc', how='left')
log(f'Merge coverage: {merged.onc_relevance.notna().sum()}/{len(merged)} drugs have PRISM metadata')
# fill missing with 0 (neutral — unknown relevance)
merged['onc_relevance'] = merged['onc_relevance'].fillna(0)

# ---------- 4. Strict oncology-filter ----------
...
log(f'Strict filter: {len(strict)}/{len(merged)} drugs retained')

# ---------- 5. Soft combined score ----------
# normalize original score to [0,1] by rank, multiply by sigmoid(onc_relevance)
merged['score_rank_frac'] = merged['score'].rank(pct=True)
merged['onc_weight'] = 1.0/(1.0 + np.exp(-merged['onc_relevance']/2.0))  # sigmoid
merged['score_combined'] = merged['score'] * merged['onc_weight'] + \
                           0.5 * merged['onc_relevance']  # small bonus from relevance
# And a pure "combined rank" for downstream use
...
# ---------- 6. Comparison report ----------
lines=[]
lines.append('# T3e oncology/phase/MOA filter — before vs after (Patch 2)\n')

lines.append('## Top-20 STRICT (oncology-tagged + Phase 1+ only)\n')
lines.append('| # | Drug | Score | MOA | Target | Phase | Onc flags |')
lines.append('|---:|---|---:|---|---|---|---|')
...
lines.append('\n## Top-20 SOFT-COMBINED (all drugs, oncology-weighted)\n')
lines.append('| # | Drug | Combined | Orig score | Onc-rel | Phase | MOA |')
lines.append('|---:|---|---:|---:|---:|---|---|')
...
# Clinical HCC drug rank in each list
clinicals = ['sorafenib','lenvatinib','regorafenib','cabozantinib','sunitinib',
             ...
             'nivolumab','pembrolizumab','atezolizumab','bevacizumab']
lines.append('\n## Clinical HCC drug ranking across the three lists\n')
lines.append('| Drug | Original rank | Strict rank | Soft rank |')
lines.append('|---|---:|---:|---:|')
for dn in clinicals:
    orig = cohort.sort_values('score', ascending=False).reset_index(drop=True)
    hit_o = orig[orig.drug.str.lower()==dn]
    hit_s = strict.reset_index(drop=True)
    hit_s = hit_s[hit_s.drug.str.lower()==dn]
    hit_m = merged.reset_index(drop=True)
    hit_m = hit_m[hit_m.drug.str.lower()==dn]
    ...
    rs = f"{r_s} ({r_s/max(len(strict),1)*100:.1f}%)" if r_s else '—'
    rm = f"{r_m} ({r_m/len(merged)*100:.1f}%)" if r_m else '—'
...
(OUT/'eval_metrics.json').write_text(json.dumps({
    ...
    'strict_top20': [{'rank':i+1,'drug':r.drug,'score':float(r.score),'phase':r.phase,
                       'moa':r.moa,'target':r.target} for i,r in enumerate(strict.head(20).itertuples())],
    'soft_top20': [{'rank':i+1,'drug':r.drug,'combined':float(r.score_combined),
                    'onc_rel':float(r.onc_relevance)} for i,r in enumerate(merged.head(20).itertuples())],
