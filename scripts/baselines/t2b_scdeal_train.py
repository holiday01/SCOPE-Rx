"""
T2b — scDEAL-style baseline: shared-encoder multi-drug regressor + DANN.

Source: pan-cancer bulk (DepMap OmicsExpressionTPM, GDSC+PRISM AUC, per-drug z-score).
Target: 20k random HCC scRNA cells (unlabeled).
Adaptation: Gradient Reversal Layer aligns source/target in 256-d latent.

Outputs:
  checkpoints/t2b_scdeal.pt
  results/t2b/eval_metrics.json
  results/t2b/scrna_drug_predictions.parquet  (cell × drug z-score)
  results/t2b/drug_level_pearson.parquet
"""
...
OUT  = ROOT/'results/t2b'; OUT.mkdir(parents=True, exist_ok=True)
CKPT = ROOT/'checkpoints'; CKPT.mkdir(parents=True, exist_ok=True)
DEV  = 'cuda'
SEED = 0
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED); torch.cuda.manual_seed_all(SEED)

HELD_OUT_HCC = ['ACH-000221','ACH-000480','ACH-000625']  # SNU398, HUH7, HEP3B217
...
BS_BULK    = 128
BS_SC      = 128
LAMBDA_DOM_MAX = 0.3  # DANN coefficient, ramped

def log(m): print(f'[{time.strftime("%H:%M:%S")}] {m}', flush=True)

# ---------- 1. Load tables ----------
log('Loading processed parquet tables …')
expr_all = pd.read_parquet(PROC/'cellline_expression_panCancer.parquet')  # (lines × genes)
...
log(f'Bulk expression: {expr_all.shape}. Drug long: {len(long_tab)}. Liver lines: {len(liver_meta)}')

# ---------- 2. Build drug × line AUC matrix (pan-cancer) ----------
# collapse multiple AUC records per (drug, line) by mean
...
# align lines order with expr_all
...
agg_z = agg.sub(agg_mean, axis=0).div(agg_std, axis=0)  # drugs × lines, z-scored per drug
Y = agg_z.T.values.astype(np.float32)           # lines × drugs
M = (~np.isnan(Y)).astype(np.float32)           # mask
Y = np.nan_to_num(Y, nan=0.0)
log(f'Y (line×drug): {Y.shape}  observed={M.sum():,.0f}  per-drug mean obs={M.sum(0).mean():.1f}')

# ---------- 4. Expression tensors ----------
X = expr_all.loc[common_lines].values.astype(np.float32)  # TPM
X = np.log1p(X)

# per-gene standardize using *bulk* (and reuse stats for scRNA)
gene_mu = X.mean(0); gene_sd = X.std(0) + 1e-6
X = (X - gene_mu) / gene_sd
log(f'X bulk: {X.shape}  after log1p+zscore')

# ---------- 5. scRNA target domain: 20k random HCC cells ----------
...
sel_sc_idx = np.array([gene_to_sc_idx[g] for g in genes])  # align to bulk gene order
n_total = a.n_obs
sample_idx = np.random.choice(n_total, size=min(N_SC_CELLS, n_total), replace=False)
sample_idx.sort()
log(f'Sampling {len(sample_idx)} of {n_total} cells …')

# read RAW counts from layers['counts'] (a.X is already log-normalised in this h5ad)
assert 'counts' in a.layers, "expected layers['counts'] with raw integers"
Xs_list = []
CHUNK = 4000
...
# raw counts → CP10K → log1p → standardize using bulk gene stats
rowsum = Xs.sum(1, keepdims=True) + 1e-6
Xs = np.log1p(Xs / rowsum * 1e4)
...
log(f'Train lines={len(X_tr)}  Held-out HCC lines={len(X_te)}  drugs={Y.shape[1]}')
...
class DrugHead(nn.Module):
    def __init__(self, d_emb, n_drugs):
        super().__init__()
        self.lin = nn.Linear(d_emb, n_drugs)
...
opt = torch.optim.Adam(list(enc.parameters())+list(dhead.parameters())+list(dom.parameters()), lr=LR, weight_decay=1e-5)
...
for ep in range(EPOCHS):
    ...
    iters = max(len(Xt)//BS_BULK, 1) * 4  # oversample a bit
    loss_r = loss_d = 0.0
    for it in range(iters):
        bi = sample_indices(len(Xt), BS_BULK)
        si = sample_indices(len(Xs_t), BS_SC)
        xb, yb, mb = Xt[bi], Yt[bi], Mt[bi]
        xs = Xs_t[si]
...
torch.save({'enc':enc.state_dict(),'dhead':dhead.state_dict(),'dom':dom.state_dict(),
            'drugs': list(agg.index),'lines_train': [ln for ln,h in zip(common_lines,held_mask) if not h],
            'lines_heldout': [ln for ln,h in zip(common_lines,held_mask) if h],
            'gene_mu':gene_mu,'gene_sd':gene_sd,'drug_mean':agg_mean.values,'drug_std':agg_std.values,
            'genes':genes},
           CKPT/'t2b_scdeal.pt')

# per-drug pearson table (final)
pearson_rows = []
enc.eval(); dhead.eval()
with torch.no_grad():
    pred_te = dhead(enc(Xte)).cpu().numpy()
...
# ---------- 10. scRNA inference ----------
log('Inference on 20k sampled scRNA cells (z-score space) …')
preds_sc = []
BATCH = 2048
with torch.no_grad():
    for i in range(0, len(Xs_t), BATCH):
        z = enc(Xs_t[i:i+BATCH])
        preds_sc.append(dhead(z).cpu().numpy())
...
log(f'mean drug Pearson on held-out HCC: {summary["mean_drug_pearson_heldout"]:.3f}')
log('== T2b done ==')
